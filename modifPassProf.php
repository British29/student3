...
include 'conn.php';
     ...
     $dup = mysqli_query($conn,"select * from professeur where email = '$email' ");
...
<head>
    <title>Tableau de bord de prof</title>
...
if (isset($_POST["button_action"])) {
    ...
    // $Newpass = $pass;
    $checkdata=" SELECT password FROM professeur WHERE id='$id' AND password = '$pass_old'  ";
    $query=mysqli_query($conn, $checkdata);
    $row = mysqli_num_rows($query);
    if($row>0)
    {
     $update = mysqli_query($conn, "UPDATE professeur SET password = '$pass_new' WHERE id='$id' AND email='$email'");
        ...
        <div class="card">
            <form method="post" name="profile_form" id="profile_form" enctype="multipart/form-data">
                ...
                <div align="center">
                    ...
                    <input type="submit" name="button_action" id="button_action" class="btn btn-success btn" value="Enregistrer" />
                </div><br>
            </form>
        </div>
    </div>
